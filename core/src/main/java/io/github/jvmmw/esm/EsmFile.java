 ...
 * land textures, pathgrids, and who sits where in a cell. Not a full game
 * simulation. Wander still walks a straight line; the graph is dump-only.
 *
...
public final class EsmFile {
    ...
    public final Map<String, EsmLevc> levc = new LinkedHashMap<>();
    ...
    public final Set<String> interiorNameSet = new HashSet<>();
    ...
    private final List<PendingPgrd> pendingPgrd = new ArrayList<>();
    private final Map<String, EsmPathgrid> pathgridByInterior = new HashMap<>();
    private final Map<Long, EsmPathgrid> pathgridByGrid = new HashMap<>();
    ...
    public static LoadedCell loadInterior(EsmReader esm, String... wanted) {
        ...
        while (esm.hasMoreRecs()) {
            ...
            } else if ("PGRD".equals(rec)) {
                file.readPathgrid(esm);
            } else {
                esm.skipRecord();
            }
        }
    ...
    public static LoadedCell loadExterior(EsmReader esm, int gridX, int gridY, int radius) {
        ...
        file.bindPathgrids();
        for (GridTile tile : center.tiles) {
            tile.pathgrid = file.pathgridForGrid(tile.gridX, tile.gridY);
        }
        center.pathgrid = file.pathgridForGrid(center.gridX, center.gridY);
    ...
    private void readPathgrid(EsmReader esm) {
        EsmPathgrid g = new EsmPathgrid();
        boolean deleted = false;
        int declaredPoints = 0;
        while (esm.hasMoreSubs()) {
            String sub = esm.getSubName();
            switch (sub) {
                case "NAME" -> g.name = esm.getHString();
                case "DATA" -> {
                    esm.getSubHeader();
                    g.gridX = esm.getI32();
                    g.gridY = esm.getI32();
                    g.granularity = esm.getI16();
                    declaredPoints = esm.getU16();
                }
                case "PGRP" -> {
                    esm.getSubHeader();
                    if (esm.leftSub() != 16 * declaredPoints) {
                        System.err.println("PGRD skip PGRP size name=" + g.name);
                        esm.skipRestOfSub();
                        skipRestOfRecord(esm);
                        return;
                    }
                    for (int i = 0; i < declaredPoints; i++) {
                        EsmPathgrid.Point p = new EsmPathgrid.Point();
                        p.x = esm.getI32();
                        p.y = esm.getI32();
                        p.z = esm.getI32();
                        p.autogenerated = esm.getU8();
                        p.connections = esm.getU8();
                        esm.skip(2);
                        g.points.add(p);
                    }
                }
                case "PGRC" -> {
                    esm.getSubHeader();
                    int size = esm.leftSub();
                    if (size < 0 || size % 4 != 0) {
                        System.err.println("PGRD skip PGRC size name=" + g.name);
                        esm.skipRestOfSub();
                        skipRestOfRecord(esm);
                        return;
                    }
                    ...
                    for (int i = 0; i < g.points.size(); i++) {
                        int conn = g.points.get(i).connections;
                        if (at + conn > n) {
                            System.err.println("PGRD skip connections name=" + g.name);
                            esm.skipRestOfSub();
                            skipRestOfRecord(esm);
                            return;
                        }
                        for (int k = 0; k < conn; k++) {
                            EsmPathgrid.Edge e = new EsmPathgrid.Edge();
                            e.v0 = i;
                            e.v1 = raw[at++];
                            g.edges.add(e);
                        }
        ...
        pendingPgrd.add(new PendingPgrd(g, deleted));
    }

    private static void skipRestOfRecord(EsmReader esm) {
        while (esm.hasMoreSubs()) {
            esm.getSubName();
            esm.skipHSub();
        }
    ...
    private void bindPathgrids() {
        for (PendingPgrd pending : pendingPgrd) {
            EsmPathgrid g = pending.grid;
            boolean interior = g.gridX == 0 && g.gridY == 0
                && interiorNameSet.contains(g.name.toLowerCase(Locale.ROOT));
            if (pending.deleted || g.points.isEmpty() || g.edges.isEmpty()) {
                if (interior) {
                    pathgridByInterior.remove(g.name.toLowerCase(Locale.ROOT));
                } else {
                    pathgridByGrid.remove(gridKey(g.gridX, g.gridY));
                }
            } else if (interior) {
                pathgridByInterior.put(g.name.toLowerCase(Locale.ROOT), g);
            } else {
                pathgridByGrid.put(gridKey(g.gridX, g.gridY), g);
            }
        }
    }

    private EsmPathgrid pathgridForInterior(String name) {
        EsmPathgrid g = pathgridByInterior.get(name.toLowerCase(Locale.ROOT));
        return g != null ? g : EsmPathgrid.NONE;
    }

    private EsmPathgrid pathgridForGrid(int x, int y) {
        EsmPathgrid g = pathgridByGrid.get(gridKey(x, y));
        return g != null ? g : EsmPathgrid.NONE;
    }

    private static final class PendingPgrd {
        final EsmPathgrid grid;
        final boolean deleted;

        PendingPgrd(EsmPathgrid grid, boolean deleted) {
            this.grid = grid;
    ...
    private void readLevc(EsmReader esm) {
        EsmLevc list = new EsmLevc();
        boolean hasList = false;
        while (esm.hasMoreSubs()) {
            String sub = esm.getSubName();
            switch (sub) {
                case "NAME" -> list.id = esm.getHString();
                case "DATA" -> {
                    esm.getSubHeader();
                    list.flags = esm.getI32();
                    esm.skipRestOfSub();
                }
                case "NNAM" -> {
                    esm.getSubHeader();
                    list.chanceNone = esm.getU8();
                    esm.skipRestOfSub();
                }
                case "INDX" -> {
                    esm.getSubHeader();
                    int count = esm.getI32();
                    esm.skipRestOfSub();
                    hasList = true;
                    for (int i = 0; i < count; i++) {
                        ...
                        EsmLevc.Entry entry = new EsmLevc.Entry();
                        entry.id = esm.getHString();
                        if (esm.hasMoreSubs()) {
                            String iv = esm.getSubName();
                            if ("INTV".equals(iv)) {
                                esm.getSubHeader();
                                entry.level = esm.getU16();
                                esm.skipRestOfSub();
                            } else {
                                esm.cacheSubName();
                            }
        ...
        if (!list.id.isEmpty()) {
            levc.put(list.id.toLowerCase(Locale.ROOT), list);
        }
    }
    ...
    private LoadedCell readCell(EsmReader esm, String[] wanted, int gridX, int gridY, int radius) {
        ...
        if (interior && !head.name.isEmpty()) {
            ...
            interiorNameSet.add(head.name.toLowerCase(Locale.ROOT));
        }
    ...
    public static final class LoadedCell {
        ...
        public Map<String, EsmLevc> levc = Map.of();
        ...
        public EsmPathgrid pathgrid = EsmPathgrid.NONE;
