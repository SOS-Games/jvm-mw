...
| `components/esm3/aipackage.hpp` | `ESM::AIPackageList` | `esm.AiPackage` list on NPC and creature | same |
...
| `apps/openmw/mwmechanics/aiescort.cpp` | `AiEscort` | front escort leads the follower and waits when they lag | rewrite |
...
| `HeightField` TES3 65×65 | `LandRecord` + `BulletWorld` land tris | rewrite |
