...
Spec: [phase56-follow.md](phase56-follow.md). **Working.**

- [x] Front `AI_F` finds the target id among loaded actors. No target: the package ends.
- [x] Farther than the package distance: walk toward them with the shared walker. Close: stand.
- [x] Duration hours end it. A `CNDT` cell name pauses it while the actor is not in that cell.
