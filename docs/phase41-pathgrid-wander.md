...
- Range is ESM `AI_W`, capped by `DebugVars.nodeWanderRadius` when that is > 0, measured from **where they stand now** so they can roam the graph. Random fallback dests use `wanderRadius` from **spawn**. A hop is allowed only when both nodes sit inside that radius of the current pose **and** of each other. Same component as the closest in-range node.
...
- Prefer dests at least 40% of the node radius away. Skip a dest another wanderer already occupies or is walking to. About one in four picks is a far trip: a node on the far side of the same connected piece, A* with no hop cap. Otherwise pick from the local cluster. A* along edges from the closest node to the current pose to the closest node to that dest. Drop the first node (already next to them). If start and goal are the same index, walk straight to the dest. Empty A* → drop that dest and pick another; if none left, fall back to a straight line this idle.
- Walk the polyline with the existing wander step / turn / `walkforward` / land-stick Z / stop within 8. On the last waypoint, pick the next dest immediately so they keep moving; pause 2–5 s only if pick fails. DebugVars speed / turn / frequency stay. Never `ModelBatch`.
