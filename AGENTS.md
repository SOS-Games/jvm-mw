...
| `npc` | Race/head/hair/skeleton/equipped parts, `wander=` / `allowed=`, then the AI package list |
| `crea` | Model, x-path, flags, scale, `wander=` / `allowed=`, then the AI package list |
| `actor` | NPC or creature by id or name (NPC wins if both match). Same text as `npc` or `crea`, including `active=` and packages |
...
- Walk-in HUD: **Cell** = Census office, **Cave** = Addamasartus, **Nix** = Punsabanit, **Guild** = Wolverine Hall Mage's Guild (door into the hall), **Town** = Seyda Neen exterior `(-2, -9)` (5×5-minus-corners land with linear 17×17 mix + shader water at −1 with refraction and underwater fog + Clear-day atmosphere, clouds, and midday sun; Census door `DODT`), **Zain** = Zainsipilu, **Manor** = Ald-ruhn, Venim Manor Right Wing (inbound door; Malsa Ules travels here), **Club** = Balmora, Council Club (Sovor Trandel follows Vadusa Sathryon). The load overlay lists `loading land`, `loading models N/M`, then `loading collision` for the spawn cell before you walk. Neighbor cells fill collision after that; WASD and wander freeze on a cell that has no Bullet yet. WASD walks on land/docks (Bullet capsule); ceilings and dock undersides stop the camera. NPCs sit on Bullet land, docks, and interior floors; they still clip shacks. Recast gathers shack tris from a load-time `CollisionMesh` snapshot, not the old tracer. Walking recenters that grid on the camera cell in the background (no freeze); the new camera cell’s Bullet cooks during that pass so swap does not stall. A small 5×5-minus-corners bar grid (bottom-right) fills per tile while a walk load is in flight, then reads `swap` for a beat. Top-right fps overlay (F4) shows frame ms, draws, culled, tex, nif, and the fattest section. **F5** toggles pathgrid spheres and edges (off at load; the spheres are built when it is on). **F6** toggles Recast walkable polys for the loaded cells (off at load; the carpet meshes are built only while it is on; sqlite first from OpenMW `navmesh.db` when present, then a background rebake of Recast tiles that fail TES cell-edge coverage; red squares mark tiles still generating; they turn green when that bake finishes; tiles stay when the 5×5 moves). **F7** collider shapes are built only while that overlay is on. Meshes outside the camera frustum are not submitted. Meshes smaller than 2 pixels are skipped on the main view. Objects farther than 7168 are skipped unless they are large (trees, shacks). Land and water stay. DDS and static NIF GPU templates intern by VFS path. The water plane follows the camera and is drawn every frame; the depth test hides it under land, so it does not vanish when cells stream. The two extra redraws run when a view ray reaches the plane before land or a building, and they stay on for a short while after that. Those passes skip NPC/creature reflections and meshes smaller than 20 pixels on the 512 map. HUD **slider / [ ] / Play** scrubs the Clear hour (stars at night).
- **E** opens/closes a non-teleport door (the swung leaf blocks WASD; closing it on you stalls the leaf), loads a named interior dest, loads an empty-`DNAM` dest as a 5×5-minus-corners around that exterior grid, plays a chest `containeropen` / `containerclose` if those kf groups exist, or takes a world item (mesh unparents and its Bullet hull goes; no inventory). Books log only (`ActionRead` GUI skipped). Fixture lights without Carry stay. A closer NPC or creature logs their AI package list and nothing else. The log starts with `active=` for the front package. Their name sits at the center of the screen while the crosshair is on them, and hides when a wall is in the way.
- NPCs are mannequins on `base_anim` / `_female` / `kna` (yaw-only, race scale). Not `NPC_.MODL`. ESM placement is a parent of `Bip01`; idle `.kf` overwrites bone locals then re-skins. The front package is the one that runs. When that is wander and its distance is greater than 0, they walk around spawn along that cell’s pathgrid when it has enough nodes (the F5 spheres); otherwise they follow the F6 carpet when Detour has a path, or a straight line if the mesh is still loading. A front travel walks to that point when it is within 7168, along the pathgrid, else the carpet, else a straight line, and then the package ends. A point farther than 7168 leaves them standing. A front follow walks toward the named actor when farther than 256 and stands when closer. No loaded target ends it. A cell name keeps them still outside that cell. Duration hours end a follow the same way they end a wander. A front escort leads the follower to the point while they are within 450, and waits with idle3 once they are farther, until they are within 250 again. No destination finishes once the follower is close. **R** uses E’s crosshair pick and puts an escort of the player in front, 2048 units along the look. Activate in front leaves them standing, even when a later wander has a distance. Distance 0 stays. When a package finishes it is dropped; if it repeats, a fresh copy goes on the back. A front wander with a duration above 0 ends after that many Clear hours (Play, ], or dragging the slider later); 0 does not end, and dragging earlier does not count. While they stand, one idle2–idle9 can play when the kf has that group, then a wanderer sets off again. Someone who stays put waits a few seconds before another gesture. Walking still plays walkforward. The time-of-day byte is stored and unused. Walk clips translate `Bip01` / `root bone` in XY — zero those so the loop does not yank them back. Test knobs live in `debug.DebugVars`: `wanderSpeed`, `creaWanderSpeed` (creatures only), `wanderTurn`, `wanderRadius` (random dest TES cap from spawn; 0 is ESM AI_W), `nodeWanderRadius` (pathgrid hop cap from where they stand; 0 is ESM AI_W), `wanderFrequency`, `idleDuration` (seconds an idle2–idle9 may play; 0 is the whole clip).
